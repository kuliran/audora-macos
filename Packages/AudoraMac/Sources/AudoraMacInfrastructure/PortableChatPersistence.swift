...
public struct PortableChatPersistence: @unchecked Sendable {
    ...
    public func loadCatalog(
        ...
        return try listEntryNames(
            ...
            .map { chatID, name in
                ...
                } catch PortableChatPersistenceError.unsupportedOlderSchema {
                    return .frozen(
    ...
    public func create(
        ...
        try revalidateProfileStatementGeneration(
            seed.aggregate.chat.profileStatementGenerationAtCreation,
    ...
    public func rename(
        ...
        let chatsDescriptor = try openDirectory(named: "chats", under: rootDescriptor)
        defer { Darwin.close(chatsDescriptor) }
        guard try entryExists(named: mutation.chatID.rawValue, under: chatsDescriptor) else {
            throw PortableChatPersistenceError.chatMissing
        }
        let chatDescriptor = try openDirectory(
            named: mutation.chatID.rawValue,
            under: chatsDescriptor
        )
        defer { Darwin.close(chatDescriptor) }
        let loaded = try loadChat(from: chatDescriptor, expectedID: mutation.chatID)
    ...
    private func openLibraryRoot(
        ...
        do {
            ...
            switch try PortableLibraryPersistence().load(from: descriptor) {
    ...
    private func revalidateProfileStatementGeneration(
        _ expected: UInt64,
        under rootDescriptor: Int32
    ) throws {
        switch try PortableLibraryPersistence().load(from: rootDescriptor) {
        case let .readWrite(authority):
            guard authority.profileHead.statementGeneration == expected else {
                throw PortableChatPersistenceError.profileStatementGenerationChanged(
                    authority.profileHead.statementGeneration
                )
            }
