...
actor NewChatCreationModule {
    ...
    func create(
        ...
        case .creationAuthorityChanged, .profileStatementGenerationChanged:
            // A requote immediately asks the same authority source for a new
            // snapshot. Its release is therefore part of the handoff: waiting
            // prevents the replacement quote from racing state that was held
            // behind this lease. Terminal outcomes still release detached so
            // presentation never inherits adapter cleanup latency.
            await lease.release()
        default:
            lease.releaseDetached()
        }
